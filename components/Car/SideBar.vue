...
<template>
    <div class="shadow border w-64 mr-10 z-30 h-[190px]">
        ...
        <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div v-if="modal.location" class="absolute border shadow left-44 p-5 top-1 -m-1 bg-white">
        ...
        <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div
              class="absolute border shadow left-52 p-5 top-1 -m-1 w-[600px] flex justify-between flex-wrap bg-white"
              v-if="modal.make"
              >
                <h4 v-for="make in makes" :key="make" class="w-1/3"  @click="onChangeMake(make)">
                    <p class="hover:text-sky-700 cursor-pointer">{{ make }}</p>
                </h4>
            </div>
        </div>
        <!-- MAKE END -->
        ...
        <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div v-if="modal.price" class="absolute border shadow left-52 p-5 top-1 -m-1 bg-white">
                ...
                <button @click="onChangePrice" class="bg-sky-500 p-1 rounded-md text-white mt-3 text-md w-full">Apply</button>
            </div>
        </div>
        <!-- PRICE END -->
    </div>
...
http://localhost:3000/city/lpool/car/Subaru?minPrice=10000&maxPrice=20000